    ...
    public class StageGroupTaskService : IStageGroupTaskService
    {
        ...
        public async Task<BaseResponse<ResponseStageGroupTaskIcon>> Create(RequestStageGroupTasNew entity, Guid userId, Guid groupId)
        {
            if (await _accountStatusGroupRepository.GetAll().AnyAsync(x => x.AccountId == userId && groupId == x.IdGroup && x.RoleAccount > RoleAccount.Default)) 
                return new StandartResponse<ResponseStageGroupTaskIcon> { Message = "User not exist in this group", StatusCode = Code.UserNotExists };


            if (await _stageGroupTaskRepository.GetAll().AnyAsync(x => x.IdGroupTask == entity.IdGroupTask && x.Name == entity.Name)) 
                return new StandartResponse<ResponseStageGroupTaskIcon> { Message = "Stage with name exists in this group", StatusCode = Code.StageGroupTaskExists };
            ...
            return new StandartResponse<ResponseStageGroupTaskIcon> { Data = stageGroupTaskIcon, StatusCode = Code.StageGroupTaskCreate };
        }

        public async Task<BaseResponse<bool>> Delete(ObjectId stageGroupTaskId, Guid userId, Guid groupId)
        {
            if (await _accountStatusGroupRepository.GetAll().AnyAsync(x => x.AccountId == userId && groupId == x.IdGroup && x.RoleAccount > RoleAccount.Default))
                return new StandartResponse<bool> { Message = "User not exist in this group", StatusCode = Code.UserNotExists };

            if (!await _stageGroupTaskRepository.GetAll().AnyAsync(x => x.Id == stageGroupTaskId))
                return new StandartResponse<bool> { Message = "Stage with name not exists in this group", StatusCode = Code.StageGroupTaskExists };

            bool result = await _stageGroupTaskRepository.Delete(x => x.Id == stageGroupTaskId);

            return new StandartResponse<bool> { Data = result, StatusCode = Code.StageGroupTaskDelete };
        }
        ...
        public async Task<BaseResponse<StageGroupTaskDTO?>> GetStageGroupTaskById(ObjectId id, Guid userId, Guid idGroupTask)
        {
            if (await _accountStatusGroupRepository.GetAll().AnyAsync(x => x.AccountId == userId && idGroupTask == x.IdGroup))
                return new StandartResponse<StageGroupTaskDTO?> { Message = "User not exist in this group", StatusCode = Code.UserNotExists };
            ...
            if (stageGroupTask == null)
                return new StandartResponse<StageGroupTaskDTO?> { StatusCode= Code.EntityNotFound, Message = "Stage group task not found" };
            ...
            return new StandartResponse<StageGroupTaskDTO> { Data = stageGroupTaskDTO, StatusCode = Code.StageGroupTaskRead}!; 
        }
        ...
        public async Task<BaseResponse<List<ResponseStageGroupTaskIcon>>> GetStagesGroupTaskIconByGroupId(Guid groupId, Guid userId)
        {
            if (await _accountStatusGroupRepository.GetAll().AnyAsync(x => x.AccountId == userId && groupId == x.IdGroup))
                return new StandartResponse<List<ResponseStageGroupTaskIcon>> { Message = "User not exist in this group", StatusCode = Code.UserNotExists };
            ...
            return new StandartResponse<List<ResponseStageGroupTaskIcon>> { Data = list, StatusCode = Code.StageGroupTaskRead};
        }
        ...
        public async Task<BaseResponse<StageGroupTaskDTO>> Update(StageGroupTaskDTO stageGroupTaskDTO, Guid userId, Guid groupId)
        {
            ...
            if (!await _stageGroupTaskRepository.GetAll().AnyAsync(x => x.IdGroupTask == stageGroupTaskDTO.IdGroupTask && x.Name == stageGroupTaskDTO.Name))
                return new StandartResponse<StageGroupTaskDTO> { Message = "Stage with name not exists in this group", StatusCode = Code.StageGroupTaskExists };
            ...
            return new StandartResponse<StageGroupTaskDTO> { Data = stageGroupTaskDTO, StatusCode = Code.StageGroupTaskUpdate };
        }
    }
}
